...
export function ModuleCard({ meta, completedCount, totalExercises }: ModuleCardProps) {
  ...
  return (
    <Link
      ...
      className="block border border-gray-200 dark:border-gray-700 rounded-lg p-5 hover:border-gray-400 dark:hover:border-gray-500 transition-colors"
    >
      <div className="flex items-start justify-between gap-4">
        <div>
          <p className="text-xs text-gray-400 dark:text-gray-500 mb-1">Module {meta.order}</p>
          <h2 className="font-semibold text-lg">{meta.title}</h2>
          <p className="text-sm text-gray-600 dark:text-gray-300 mt-1">{meta.description}</p>
          <p className="text-xs text-gray-400 dark:text-gray-500 mt-2">{meta.estimatedMinutes} min</p>
        </div>
        {completedCount > 0 && (
